...
export class Interval {
    ...
    overlaps(that: Interval): boolean {
        return (
            (this.min <= that.min && that.min <= this.max) ||
    ...
    union(that: Interval): Interval {
        if (!this.overlaps(that)) {
            throw new Error(
                "The intervals must overlap to use union this way -pekd",
            );
        }
        return new Interval(
    ...
    intersect(that: Interval): Interval {
        if (!this.overlaps(that)) {
            throw new Error(
                "The intervals must overlap to use intersect this way -pekd",
            );
        }
        return new Interval(
...
export class Adjusted<Type> {
    ...
    constructor(
        type: Type,
        maxValidRow: number = Number.MAX_SAFE_INTEGER,
        isUnchanged = true,
    ) {
        this.type = type;
...
export class SuperRARef {
    ...
    insertRowCols(
        ...
        let upper: number;
        if (insertRow) {
            const result: { newRef: number; upper: number } =
                this.insertRowColsRaw(R, N, r, this.rowAbs, this.rowRef);
            newRef = result.newRef;
            upper = result.upper;
            const newRARef: SuperRARef = new SuperRARef(
                this.colAbs,
                this.colRef,
                this.rowAbs,
                newRef,
            );
            return new Adjusted<SuperRARef>(
                newRARef,
                upper,
                this.rowRef === newRef,
            );
            ...
            const result: { newRef: number; upper: number } =
                this.insertRowColsRaw(R, N, r, this.colAbs, this.colRef);
            newRef = result.newRef;
            upper = result.upper;
            const newRARef: SuperRARef = new SuperRARef(
                this.colAbs,
    ...
    move(deltaCol: number, deltaRow: number): SuperRARef {
        return new SuperRARef(
            this.colAbs,
            this.colAbs ? this.colRef : this.colRef + deltaCol,
            this.rowAbs,
            this.rowAbs ? this.rowRef : this.rowRef + deltaRow,
        );
    }
    ...
    show(col: number, row: number, format: Formats): string {
        ...
        switch (format.getRefFmt()) {
            case "A1": {
                const celladdr: RARefCellAddress = new RARefCellAddress(
                    this,
                    col,
                    row,
                );
                return (
                    (this.colAbs ? "$" : "") +
                    celladdr.columnName(celladdr.col) +
                    (this.rowAbs ? "$" : "") +
                    (celladdr.row + 1)
                );
            }
            case "R1C1":
                return (
                    "R" +
                    this.relAbsFormat(this.rowAbs, this.rowRef, 1) +
                    "C" +
                    this.relAbsFormat(this.colAbs, this.colRef, 1)
                );
            case "C0R0":
                return (
                    "C" +
                    this.relAbsFormat(this.colAbs, this.colRef, 0) +
                    "C" +
                    this.relAbsFormat(this.rowAbs, this.rowRef, 0)
                );
            default:
    ...
    equals(that: SuperRARef): boolean {
        return (
            that != null &&
            this.colAbs === that.colAbs &&
            this.colRef === that.colRef &&
            this.rowAbs === that.rowAbs &&
            this.rowRef === that.rowRef
        );
    }
}
...
export class R1C1RARef extends SuperRARef {
    ...
    constructor(r1c1: string) {
        ...
        let result: { value: number; index: number } = this.parseIntWithIndex(
            this.r1c1,
            i,
        );
...
export class SuperCellAddress {
     ...
     * @param ca2
     */
    static normalizeArea(
        ca1: SuperCellAddress,
        ca2: SuperCellAddress,
    ): { ulCa: SuperCellAddress; lrCa: SuperCellAddress } {
        ...
        return {
            ulCa: new SuperCellAddress(minCol, minRow),
            lrCa: new SuperCellAddress(maxCol, maxRow),
        };
    }

    offset(offset: SuperCellAddress): SuperCellAddress {
        return new SuperCellAddress(
            this.col + offset.col,
            this.row + offset.row,
        );
    }
...
export class FullCellAddress {
    ...
    constructor(
        sheet: Sheet,
        cellAddress?: SuperCellAddress | null,
        col?: number,
        row?: number,
        a1string?: string,
    ) {
    ...
    equals(that: FullCellAddress | unknown): boolean {
        if (that instanceof FullCellAddress) {
            return (
                this.cellAddress.equals(that.cellAddress) &&
    ...
    public Eval(): Value | null {
        ...
        if (cell != null) {
            return cell.Eval(
                this.sheet,
                this.cellAddress.col,
                this.cellAddress.row,
            );
        }
        return null;
    }
...
export class SupportSet {
    ...
    addSupport(
        ...
        const range: SupportRange = SupportRange.make(
            suppSheet,
            suppCols,
            suppRows,
        );
...
export abstract class SupportRange {
    static make(
        ...
        if (
            colInterval.min === colInterval.max &&
            rowInterval.min === rowInterval.max
        ) {
...
export class SupportArea extends SupportRange {
        ...
        if (this.contains(sheet, col, row)) {
            if (this.rowInterval.min < row) {
                set.add(
                    SupportArea.make(
                        sheet,
                        new Interval(col, col),
                        new Interval(this.rowInterval.min, row - 1),
                    ),
                );
            }
            if (row < this.rowInterval.max) {
                set.add(
                    SupportArea.make(
                        sheet,
                        new Interval(col, col),
                        new Interval(row + 1, this.rowInterval.max),
                    ),
                );
            }
            if (this.colInterval.min < col) {
                set.add(
                    SupportArea.make(
                        sheet,
                        new Interval(this.colInterval.min, col - 1),
                        this.rowInterval,
                    ),
                );
            }
            if (col < this.colInterval.max) {
                set.add(
                    SupportArea.make(
                        sheet,
                        new Interval(col + 1, this.colInterval.max),
                        this.rowInterval,
                    ),
                );
            }
    ...
    forEachSupported(
        ...
        if (
            SupportArea.idempotentForeach &&
            this.count > SupportArea.alreadyVisited.length + 1
        ) {
            for (let i = 0; i < SupportArea.alreadyVisited.length; i++) {
                ...
                if (this.overlaps(old)) {
                    ...
                    if (overlap.count === this.count) {
                        ...
                        this.colInterval.equals(old.colInterval) &&
                        this.rowInterval.overlaps(old.rowInterval)
                    ) {
                        SupportArea.alreadyVisited[i] = new SupportArea(
                            this.sheet,
                            this.colInterval,
                            this.rowInterval.union(old.rowInterval),
                        );
                        this.forEachExcept(overlap, act);
                        return;
                    } else if (
                        this.rowInterval.equals(old.rowInterval) &&
                        this.colInterval.overlaps(old.colInterval)
                    ) {
                        SupportArea.alreadyVisited[i] = new SupportArea(
                            this.sheet,
                            this.colInterval.union(old.colInterval),
                            this.rowInterval,
                        );
                        this.forEachExcept(overlap, act);
                        return;
    ...
    private forEachExcept(
        ...
        if (this.rowInterval.min < overlap.rowInterval.min)
            ...
            SupportArea.forEachInArea(
                this.sheet,
                overlap.colInterval,
                new Interval(this.rowInterval.min, overlap.rowInterval.min - 1),
                act,
            );
        if (overlap.rowInterval.max < this.rowInterval.max)
            ...
            SupportArea.forEachInArea(
                this.sheet,
                overlap.colInterval,
                new Interval(overlap.rowInterval.max + 1, this.rowInterval.max),
                act,
            );
        if (this.colInterval.min < overlap.colInterval.min)
            ...
            SupportArea.forEachInArea(
                this.sheet,
                new Interval(this.colInterval.min, overlap.colInterval.min - 1),
                this.rowInterval,
                act,
            );
        if (overlap.colInterval.max < this.colInterval.max)
            ...
            SupportArea.forEachInArea(
                this.sheet,
                new Interval(overlap.colInterval.max + 1, this.colInterval.max),
                this.rowInterval,
                act,
            );
    }
    ...
    override contains(sheet: Sheet, col: number, row: number): boolean {
        return (
            this.sheet === sheet &&
            this.colInterval.contains(col) &&
            this.rowInterval.contains(row)
        );
    }
    ...
    overlaps(that: SupportArea): boolean {
        return (
            this.sheet === that.sheet &&
            this.colInterval.overlaps(that.colInterval) &&
            this.rowInterval.overlaps(that.rowInterval)
        );
    }
    ...
    overlap(that: SupportArea): SupportArea {
        ...
        return new SupportArea(
            this.sheet,
            this.colInterval.intersect(that.colInterval),
            this.rowInterval.intersect(that.rowInterval),
        );
    }

    override toString(): string {
        const ulCa: SuperCellAddress = new SuperCellAddress(
            this.colInterval.min,
            this.rowInterval.min,
        );
        const lrCa: SuperCellAddress = new SuperCellAddress(
            this.colInterval.max,
            this.rowInterval.max,
        );
