...
export abstract class Expr {
    ...
    public abstract VisitRefs(
        refSet: RefSet,
        refAct: (cellRef: CellRef) => void,
        areaAct: (cellArea: CellArea) => void,
    ): void;
    ...
    public AddToSupportSets(
        ...
        this.VisitRefs(
            new RefSet(),
            (cellRef: CellRef) =>
                cellRef.AddToSupport(supported, col, row, cols, rows),
            (cellArea: CellArea) =>
                cellArea.AddToSupport(supported, col, row, cols, rows),
        );
    }

    // Apply act, once only, to the full cell address of each cell referred from expression
    public ForEachReferred(
        sheet: Sheet,
        col: number,
        row: number,
        act: (fullCellAddr: FullCellAddress) => void,
    ): void {
        this.VisitRefs(
            new RefSet(),
    ...
    public abstract DependsOn(
        ...
        dependsOn: (fca: FullCellAddress) => void,
    ): void;
...
abstract class Const extends Expr {
    ...
    public override DependsOn(
        here: FullCellAddress,
        dependsOn: (fullCellAddress: FullCellAddress) => void,
    ): void {}
...
export class ErrorConst extends Const {
    ...
    public static readonly refError: ErrorConst = new ErrorConst(
        ErrorValue.refError,
    );
...
type functionType = (
    ...args: (string | number | boolean | ErrorValue | null | nestedArgs)[]
...
export class FunCall extends Expr {
    ...
    private static CHOOSE(es: Expr[]) {
        const func: functionType = (...args) => {
            if (
                ((args[0] as Value).ToObject() as number) >= 1 &&
                ((args[0] as Value).ToObject() as number) <= args.length
            ) {
                return args[0] as
                    | string
                    | number
    ...
    public static Make(name: string, es: Expr[]): Expr {
        ...
        if (name === "CHOOSE") {
            return this.CHOOSE(es);
        }
        if (name === "NOTEQUALS") {
            return this.NOTEQUALS(es);
        }
        if (name === "GEQ") {
            return this.GEQ(es);
        }
        if (name === "GEQUALS") {
            return this.GEQUALS(es);
        }
        if (name === "LEQUALS") {
            return this.LEQUALS(es);
        }
        if (name === "LEQ") {
            return this.LEQ(es);
        }
        if (name === "CONCATENATE") {
            return this.CONCATENATE(es);
        }
        if (name === "ARRAY") {
    ...
    public override Eval(sheet: Sheet, col: number, row: number): Value {
        ...
        if (this.nonStrict) {
            if (!this.isChoose) {
                ...
                return this.es[
                    this.es[0].Eval(sheet, col, row).ToObject() as number
                ].Eval(sheet, col, row);
            }
        }
    ...
    public static getExprValues(
        ...
        return es.map((expr) => {
            if (expr instanceof ExprArray) {
                // E.g. [2,4] in formula cell in GUI
                return FunCall.getExprValues(
                    sheet,
                    col,
                    row,
                    expr.GetExprArray(),
            ...
            if (value instanceof ArrayView) {
                // E.g. A1:C3 in formula cell in GUI
    ...
    public override InsertRowCols(
        ...
        for (let i = 0; i < this.es.length; i++) {
            const ae: Adjusted<Expr> = this.es[i].InsertRowCols(
                modSheet,
                thisSheet,
                R,
                N,
        ...
        return new Adjusted<Expr>(
            new FunCall(this.function, newEs),
            upper,
            same,
        );
    }
    ...
    public override Show(
        ...
        if (pre === 0) {
            ...
            if (this.es.length === 2) {
                ...
                stringArray.push(
                    this.function.name === "NEG" ? "-" : this.function.name,
                );
                stringArray.push(this.es[0].Show(col, row, pre, fo));
            } else {
...
export class CellRef extends Expr {
    ...
    public override Eval(sheet: Sheet, col: number, row: number): Value {
        const cell: Cell | null = (this.sheet ?? sheet).Get(
            this.raref.address(col, row).col,
            this.raref.address(col, row).row,
        )!; // ca.col = 0, ca.row = 0
    ...
    public GetAbsoluteAddr(
        sheet: Sheet | FullCellAddress,
        col?: number,
        row?: number,
    ): FullCellAddress {
        if (sheet instanceof FullCellAddress) {
            return this.GetAbsoluteAddr(
                sheet.sheet,
                sheet.cellAddress.col,
                sheet.cellAddress.row,
            );
        } else {
            return new FullCellAddress(
                this.sheet ?? sheet,
                this.raref.address(col as number, row as number),
            );
        }
    }
    ...
    public override InsertRowCols(
        ...
        if (this.sheet == modSheet || (this.sheet == null && thisSheet)) {
            const adj: Adjusted<SuperRARef> = this.raref.insertRowCols(
                R,
                N,
                r,
                doRows,
            ...
            return new Adjusted<Expr>(
                new CellRef(this.sheet, adj.type),
                adj.maxValidRow,
                adj.isUnchanged,
            );
        } else {
    ...
    public AddToSupport(
        ...
        [referredCols, supportedCols] = CellRef.RefAndSupp(
            this.raref.colAbs,
            ca,
            c1,
            c2,
        );
        [referredRows, supportedRows] = CellRef.RefAndSupp(
            this.raref.rowAbs,
            ra,
            r1,
            r2,
    ...
    private static RefAndSupp(
        abs: boolean,
        ra: number,
        r1: number,
        r2: number,
    ): [Interval, (arg: number) => Interval] {
...
export class CellArea extends Expr {
    ...
    constructor(
        ...
        if (ulColAbs instanceof SuperRARef) {
            ...
            this.ul = new SuperRARef(
                ulColAbs as boolean,
                ulColRef as number,
                ulRowAbs as boolean,
                ulRowRef as number,
            );
            this.lr = new SuperRARef(
                lrColAbs as boolean,
                lrColRef as number,
                lrRowAbs as boolean,
                lrRowRef as number,
    ...
    public MakeArrayView(
        fca: FullCellAddress | Sheet,
        col?: number,
        row?: number,
    ): ArrayView {
        if (fca instanceof FullCellAddress) {
            return this.MakeArrayView(
                fca.sheet,
                fca.cellAddress.col,
                fca.cellAddress.row,
            );
    ...
    ApplyToFcas(
        ...
        ArrayView.Make(ulCa, lrCa, this.sheet ?? sheet).Apply((val: Value) => {
            const fullCellAddr: FullCellAddress =
                val as unknown as FullCellAddress;
            act(fullCellAddr);
        });
    }
    ...
    public override Move(deltaCol: number, deltaRow: number): Expr {
        return new CellArea(
            this.sheet,
            this.ul.move(deltaCol, deltaRow),
            this.lr.move(deltaCol, deltaRow),
        );
    }
    ...
    public override InsertRowCols(
        ...
        if (this.sheet == modSheet || (this.sheet == null && thisSheet)) {
            const ulNew: Adjusted<SuperRARef> = this.ul.insertRowCols(
                R,
                N,
                r,
                doRows,
            );
            const lrNew: Adjusted<SuperRARef> = this.lr.insertRowCols(
                R,
                N,
                r,
                doRows,
            ...
            return new Adjusted<Expr>(
                new CellArea(this.sheet, ulNew.type, lrNew.type),
                upper,
                ulNew.isUnchanged && lrNew.isUnchanged,
            );
        } else {
            return new Adjusted<Expr>(this);
        }
    ...
    public AddToSupport(
        ...
        let supportedCols: (arg: number) => Interval;
        let supportedRows: (arg: number) => Interval;
        const ra = this.ul.rowRef,
            rb = this.lr.rowRef,
            r1 = row,
            r2 = row + rows - 1;
        const ca = this.ul.colRef,
            cb = this.lr.colRef,
            c1 = col,
            c2 = col + cols - 1;

        [referredRows, supportedRows] = CellArea.RefAndSupp(
            this.ul.rowAbs,
            this.lr.rowAbs,
            ra,
            rb,
        ...
        [referredCols, supportedCols] = CellArea.RefAndSupp(
            this.ul.colAbs,
            this.lr.colAbs,
            ca,
            cb,
    ...
    private static RefAndSupp(
        ...
        if (ulAbs) {
            if (lrAbs) {
                ...
                return [
                    new Interval(Math.min(ra, r1 + rb), Math.max(ra, r2 + rb)),
                    (r) =>
                        ra < r
    ...
    public override DependsOn(
        ...
        this.ApplyToFcas(
            here.sheet,
            here.cellAddress.col,
            here.cellAddress.row,
            dependsOn,
        );
    }
    ...
    public equals(that: CellArea): boolean {
        return (
            that != null && this.ul.equals(that.ul) && this.lr.equals(that.lr)
        );
    }
