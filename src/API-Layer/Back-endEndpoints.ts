...
export function ParseToActiveCell(content: string): void {
    ...
    if (!cellToBeAdded) {
        console.debug(
            "[WorkbookIO] ParseToActiveCell cellToBeAdded not found!",
        );
        return;
    }
    WorkbookManager.getWorkbook()
        .getSheet(WorkbookManager.getActiveSheetName())
        ?.SetCell(cellToBeAdded, cellCol, cellRow);
}
...
export function GetRawCellContent(cellID: string): string | null {
    ...
    const cellHTML = document.getElementById(
        (colChar + (cellRow + 1)) as string,
    );
    if (!cellHTML) {
...
export function EvalCellsInViewport(): void {
    ...
    if (sheet) {
        let cells = Array.from(
            document
                .getElementById("gridBody")!
                .querySelectorAll(
                    'div.Cell[contenteditable="true"]:not(.hide)',
                ),
        ) as HTMLElement[];
        ...
        for (let col: number = 0; col < cells.length; col++) {
            if (cells[col]) {
                ...
                if (cellHTML != null) {
                    ...
                    if (
                        cells[col].id == WorkbookManager.getActiveCell() &&
                        !(cell instanceof ArrayFormula)
                    ) {
                        continue;
                    }
...
export function GetSupportsInViewPort(col: number, row: number): string[] {
    ...
    WorkbookManager.getActiveSheet()
        ...
        ?.ForEachReferred(
            ...
            col,
            row,
            (addr: FullCellAddress) => {
                supports.push(
                    (numberToLetters(addr.cellAddress.col + 1) +
                        (addr.cellAddress.row + 1)) as string,
                );
            },
        );
...
export function GetDependenciesInViewPort(col: number, row: number): string[] {
    ...
    WorkbookManager.getActiveSheet()
        ?.Get(col, row)
        ?.ForEachSupported((_sheet: Sheet, col: number, row: number) => {
...
export function ParseCellToBackend(
    ...
    WorkbookManager.getWorkbook()
        ?.getSheet(WorkbookManager.getActiveSheetName())
        ?.SetCell(cellToBeAdded, columnIndex, rowIndex);
    WorkbookManager.getWorkbook().Recalculate();
    return true;
}

export function HandleArrayResult(
    columnIndex: number,
    rowIndex: number,
): boolean {
    //Handle Array Results for different cells.
    const cell = WorkbookManager.getWorkbook()
        ?.getSheet(WorkbookManager.getActiveSheetName())
        ?.Get(columnIndex, rowIndex);
    if (!cell) return false; // Check that the cell is not null
    const result = cell.Eval(
        WorkbookManager.getWorkbook()?.getSheet(
...
export function HandleArrayFormula(
    columnIndex: number,
    rowIndex: number,
): boolean {
    const checkCell = WorkbookManager.getWorkbook()
        ?.getSheet(WorkbookManager.getActiveSheetName())
        ?.Get(columnIndex, rowIndex);
