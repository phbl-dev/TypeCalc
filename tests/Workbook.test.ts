...
describe("Workbook", () => {
    ...
    test("RecordCellChange", () => {
        ...
        expect(workbook.GetEditedCells()[0].cellAddress).toBe(
            sheet.workbook.GetEditedCells()[0].cellAddress,
        ); // Ensure that the edit occurred in the intended position of the sheet.
    });
    ...
    test("AddToQueue", () => {
        ...
        expect(workbook.GetAwaitsEvaluation()[0].toString()).toBe(
            "TestSheet!A1",
        ); // And the cell is in fact in position A1 (corresponding to 0,0) and from sheet "TestSheet"
        ...
        expect(workbook.GetAwaitsEvaluation()[0].toString()).toBe(
            "TestSheet!A1",
        ); // The old cell is still at index 0 as it should be
        expect(workbook.GetAwaitsEvaluation()[1].toString()).toBe(
            "TestSheet!B1",
        ); // And the newly added cell is at index 1 as expected.
    });
